...
class Database(DatabaseComponents, metaclass=Singleton):
    ...
    Attributes:
        _DATA (pd.DataFrame): The main DataFrame storing the data fetched from different sources.
        _seeken_dates (dict): A dictionary that keeps track of the open and close dates for each ticker.
    """
    ...
    def _fetch_brapi(self, ticker: str, open_date, close_date):
        ...
        df = pd.DataFrame(info, index=dates)
        df = df.loc[open_date:close_date]
        self._DATA = pd.concat([df, self._DATA], axis=1)
        return True

    def _fetch_prices(self, ticker: str, open_date, close_date):
        """
        Fetches price data for a ticker using Yahoo Finance first, then BRAPI if Yahoo fails, and updates the _DATA DataFrame.
        ...
        Raises:
            ValueError: If data could not be fetched from both Yahoo Finance and BRAPI.
        """
        fetch = self._fetch_yf(ticker, open_date, close_date)
        if not fetch:
            fetch = self._fetch_brapi(ticker, open_date, close_date)
        if not fetch:
            raise ValueError(f'Não foi possivel resgatar os valores do ativo {ticker}')
        self._add_seeken_dates(ticker, open_date, close_date)

    def _fetch_currencies(self, ticker: str, open_date, close_date):
        """
        Fetches currency exchange rate data using Yahoo Finance and updates the _DATA DataFrame.
        ...
        candles.index.names = ['date']
        candles = candles.tz_localize(None)
        self._DATA = pd.concat([candles[[ticker+'_close']], self._DATA], axis=1)
    ...
    def _check_index(self, ticker):
        ...
        Returns:
            tuple: Configuration details, such as whether the data is for an index, currency, etc.
        """
        is_volatility = VOLATILITY_INFO.get(ticker)
        is_return = RETURN_INFO.get(ticker)
        is_currency = CURRENCIES.get(ticker)
        is_cdi = ticker == 'CDI'
        is_pib = ticker == 'PIBBR'
        is_index = INDEXES.get(ticker)
        return is_volatility, is_return, is_currency, is_cdi, is_pib, is_index
    ...
    def _allow_changes(self, ticker, open_date, close_date):
        ...
        Returns:
            tuple: Contains a boolean indicating if data should be fetched, and the adjusted open and close dates.
        """
        seeken_dates = self._seeken_dates.get(ticker)
        if seeken_dates:
            open_date_to_fetch = seeken_dates['start']
            close_date_to_fetch = seeken_dates['close']
            if open_date < open_date_to_fetch:
                open_date_to_fetch = open_date
            if close_date > close_date_to_fetch:
                close_date_to_fetch = close_date
            if open_date_to_fetch < seeken_dates['start'] or close_date_to_fetch > seeken_dates['close']:
                return True, open_date_to_fetch, close_date_to_fetch
            return False, None, None
        return True, open_date, close_date

    def _add_assets(self, ticker, open_date, close_date):
        """
        Adds the data for the specified ticker and date range to the internal DataFrame (_DATA).
        ...
        Raises:
            ValueError: If the ticker is not recognized.
        """
        is_volatility, is_return, is_currency, is_cdi, is_pib, is_index = self._check_index(ticker)
        if is_cdi:
            self._fetch_CDI(open_date, close_date)
        elif is_pib:
            self._fetch_PIB_BR(open_date, close_date)
        elif is_index or is_currency:
            self._fetch_prices(ticker, open_date, close_date)
            if is_index:
                self._DATA[ticker+'_volatility'] = self._DATA[ticker+'_close'].pct_change().rolling(10).std()
                self._DATA[ticker+'_return'] = self._DATA[ticker+'_close'].pct_change()
                self._DATA[ticker+'_cumulative_return'] = (self._DATA[ticker+'_return'] + 1).cumprod()
            if is_currency:
                self._DATA[ticker+'_return'] = self._DATA[ticker+'_close'].pct_change()
        elif is_return:
            self._fetch_sgs(ticker, open_date, close_date)
        elif is_volatility:
            self._fetch_sgs(ticker, open_date, close_date)
            self._DATA[ticker+'_volatility'] = self._DATA[ticker+'_close'].pct_change().rolling(10).std()
        else:
            raise ValueError(f'{ticker} não é um ativo válido')
        self._add_seeken_dates(ticker, open_date, close_date)

    def get_info(self, tickers: list, open_date: str = None, close_date: str = None, info='close'):
        """
        Retrieves the specified information (e.g., 'close', 'ohlcv') for a list of tickers.
        ...
        Returns:
            pd.DataFrame: DataFrame containing the requested data.
        """
        open_date, close_date = DateProcessor().dates_in_line(open_date, close_date)
        if type(tickers) is str:
            tickers = [tickers]
        for ticker in tickers:
            allow_changes, open_date_to_fetch, close_date_to_fetch = self._allow_changes(ticker, open_date, close_date)
            if allow_changes:
                self._add_assets(ticker, open_date_to_fetch, close_date_to_fetch)
        if info == 'close':
            cols = [ticker+'_close' for ticker in tickers]
        elif info == 'ohlcv':
            cols = []
            for ticker in tickers:
                cols.extend([ticker+'_close', ticker+'_open', ticker+'_high', ticker+'_low', ticker+'_volume'])
        return self._DATA[cols].loc[open_date:close_date]
